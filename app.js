...
// helpers for the handlebar template platform
...
initDb(config.databaseConnectionString, async (err, db) => {
    ...
    try {
        ...
        app.emit('appStarted');
        console.log(colors.green('DaLat Book is running on host: http://localhost:' + app.get('port')));
    } catch (ex) {
        console.error(colors.red('Error starting DaLat Book app:' + err));
        process.exit(2);
    }
});
